const dbName = `union${  'production' !== process.env.NODE_ENV ? `_${  process.env.NODE_ENV}` : ''}`;
...
const migrator = async function () {
  // Get some env info
  let lastMigration = 0;
  if (fs.existsSync(path.resolve(__dirname, '_migration'))) {
    lastMigration = parseInt(fs.readFileSync(path.resolve(__dirname, '_migration')));
  }
